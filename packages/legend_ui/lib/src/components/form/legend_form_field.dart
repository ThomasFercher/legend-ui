...
///
/// The [builder] receives the field's [LegendFormFieldState]; the consumer
/// renders whatever input they like, calls
/// [LegendFormFieldState.didChange] on edits, and displays
/// [LegendFormFieldState.error]:
///
/// ```dart
...
///   builder: (context, field) => LegendSwitch(
///     value: field.value ?? false,
///     onChanged: field.didChange,
///   ),
/// )
/// ```
class LegendFormField<T> extends StatefulWidget {
  const LegendFormField({
    required this.name,
  ...
  final Widget Function(BuildContext context, LegendFormFieldState<T> field)
  builder;

  @override
  LegendFormFieldState<T> createState() => LegendFormFieldState<T>();
}

/// The state handed to [LegendFormField.builder] — current [value], displayed
/// [error], and [didChange] to report edits.
class LegendFormFieldState<T> extends State<LegendFormField<T>>
  ...
  void didChangeDependencies() {
    ...
    final form = LegendForm.maybeOf(context);
    if (form != _form) {
  ...
  @override
  void didUpdateWidget(LegendFormField<T> oldWidget) {
