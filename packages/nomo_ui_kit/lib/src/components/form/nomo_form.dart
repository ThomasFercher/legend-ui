...
/// The contract a field implements to participate in a [NomoForm].
///
/// Kit fields (`NomoTextField` via `formField`) and consumer fields (via
/// `NomoFormField`) implement this; fully custom widgets may too. A handle
...
abstract class NomoFormFieldHandle {
  ...
  /// error. Called by the controller on [NomoFormController.validate],
...
/// Owns a [NomoForm]'s field registry and validity model.
///
...
class NomoFormController {
  final Map<String, NomoFormFieldHandle> _fields = {};
  ...
  void register(NomoFormFieldHandle field) {
...
/// Scopes a [NomoFormController] to a subtree.
///
/// Descendant fields find the controller via [NomoForm.maybeOf] and manage
...
/// owns one internally.
class NomoForm extends StatefulWidget {
  const NomoForm({required this.child, this.controller, super.key});

  /// External controller; the form creates (and disposes) its own if null.
  final NomoFormController? controller;

  final Widget child;

  /// The nearest enclosing form's controller, or null when not in a form.
  static NomoFormController? maybeOf(BuildContext context) {
    return context
        .dependOnInheritedWidgetOfExactType<_NomoFormScope>()
        ?.controller;
  }

  /// The nearest enclosing form's controller.
  static NomoFormController of(BuildContext context) {
    final controller = maybeOf(context);
    assert(controller != null, 'No NomoForm found in context.');
    return controller!;
  }

  @override
  State<NomoForm> createState() => _NomoFormState();
}

class _NomoFormState extends State<NomoForm> {
  NomoFormController? _ownController;

  NomoFormController get _controller =>
      widget.controller ?? (_ownController ??= NomoFormController());
  ...
  Widget build(BuildContext context) {
    return _NomoFormScope(controller: _controller, child: widget.child);
  }
}

class _NomoFormScope extends InheritedWidget {
  const _NomoFormScope({required this.controller, required super.child});

  final NomoFormController controller;

  @override
  bool updateShouldNotify(_NomoFormScope oldWidget) =>
