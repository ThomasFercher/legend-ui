...
typedef NomoValidatorFn = String? Function(String? value);

/// The stock validators the legacy docs always claimed existed — now real
/// (DESIGN.md §3: "an actual `NomoValidator` set").
///
...
/// )
/// ```
abstract final class NomoValidator {
  ...
  /// Empty values pass — combine with [required] via [compose].
  static NomoValidatorFn minLength(int length, {String? message}) {
    return (value) {
      if (value == null || value.isEmpty) return null;
  ...
  static NomoValidatorFn maxLength(int length, {String? message}) {
    return (value) {
  ...
  /// all pass.
  static NomoValidatorFn compose(List<NomoValidatorFn> validators) {
    return (value) {
