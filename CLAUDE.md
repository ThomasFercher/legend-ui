...
### Linting: `very_good_analysis` is strict — reach for `dart fix` first

Every package includes `package:very_good_analysis` (see `analysis_options.yaml`), so the bar is much higher than the default lints, and **`flutter analyze` must be completely clean** — even `info`-level findings are gate failures: the docs-site launch configs refuse to start on any finding, and CI runs `flutter analyze` with no severity filter. Generated `*.g.dart` files are analyzer-excluded, so lint findings only come from hand-written code (and, for the generator, the *template strings* it emits — fix those in the emitter, then regenerate).

Most findings this repo produces are mechanical and **auto-fixable** — run this BEFORE hand-editing or committing, and before you even read the analyzer output:

```bash
dart fix --apply        # from the repo root, or inside a specific package
dart format .           # re-format after fixes
flutter analyze         # confirm zero findings remain
```

`dart fix --dry-run` previews without writing. `dart fix` resolves the vast majority of very_good_analysis nits with zero thought — do not hand-edit these one-by-one. Common ones it fixes automatically, all seen in this repo:
- `cascade_invocations` — consecutive calls on the same receiver must use `..` cascades (very common in tests: `controller..setDark(value: true)..setSeed(seed)`).
- `unnecessary_import` — e.g. importing `dart:ui` for `Color` when `package:flutter/foundation.dart` (or `widgets.dart`) already re-exports it.
- `directives_ordering` (alphabetized imports/exports — the barrel `legend_ui.dart` is strictly sorted), `prefer_const_constructors`, `require_trailing_commas`, `unnecessary_late`, `omit_local_variable_types`, `prefer_final_locals`.

A few classes `dart fix` cannot resolve and you must fix by hand: real logic issues, `public_member_api_docs` (disabled for now — see the analysis_options note), naming, and anything needing a judgement call. When a finding is a deliberate, justified exception, use a scoped `// ignore: rule_name` with a one-line reason — never a file-wide `ignore_for_file` on hand-written code (generated files carry their own `ignore_for_file` headers, which is fine).
