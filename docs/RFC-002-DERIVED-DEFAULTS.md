...
| **E — custom style classes + granular rebuilds + wiring (R6-final, R12, R13)** | custom `@Style` classes replace `LegendStates<T>` (kit predefines `InteractiveColors`); per-field `listen:` opt-out, `_field` accessors, `ValueListenable` selectors; extensions-first wiring with the opt-in `_$XBase` two-arg build; terse Ref renames via `refName:` (`ColorRef`, `SizeRef`, `TextRef`, `ShadowRef`, `StateRef`, `TokenRef`) | in progress (2026-07-10): kit migration keeps the 25 per-state pins from step C unchanged; consumer fixture gains a custom style class as the workflow proof |
