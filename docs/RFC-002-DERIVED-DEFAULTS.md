...
> ```
>
> R12 aspects register inside the hook/getter resolution; per-field `_<field>(context)` accessors unchanged. If no State class is found in the widget's file, the getter extension is simply not emitted — the hook covers everything. **Additionally a mixin form is emitted** (`mixin _$XThemeState on State<X> { XTheme get theme => widget._theme(context); }`): the explicit opt-in — needs no State detection, reads as a real inherited member, overridable; the auto-extension and the mixin coexist (instance members win over extensions). No custom elements or base classes required, no bootstrap edge, full interop with default Flutter widgets by construction.
>
> **The base form stays as an opt-in** *(2026-07-10, directed)*: for stateless widgets the part also always emits `_$XBase` (extending the kit's `LegendStatelessWidget<XTheme>`, abstract getters over the themed fields, `resolveThemeOf` with aspect registration), so an author who wants the two-arg signature simply writes `class X extends _$XBase { Widget build(BuildContext context, XTheme theme) {...} }`. Plain-widget forms remain the default; the kit hierarchy exists solely to serve this opt-in. Stateful stays getter-only (no two-arg variant).
...
| **C2 — Ref catalogs (R10 amendment)** | in-annotation token defaults: generated `<Class>Ref` const tear-off catalogs + top-level composite tear-offs | **landed** (2026-07-10): `@LegendTokenData(mountedAt:)` emits a const tear-off catalog per token class (`''` root sentinel closes the set over the whole base theme — six catalogs incl. `LegendTokensRef`); 38 one-hop defaults migrated into catalog members and 33 composites into private top-level functions — zero adjacent hand-written tear-off statics remain in component files; goldens extended (legend_gen 87 → 95, kit 144 → 148) |
| **D — playground & docs CMS** | manifest-driven configurator over every variable; Theme reference page; `LegendThemeController` in kit | **half-landed, half parked** (2026-07-10): the kit half landed — `LegendThemeController` (R8) cherry-picked to `rewrite` at `668cb0f` with its 7-test suite (seed-or-tokens source, single dark flag, typed no-op-on-equal mutators). The example half (manifest-driven configurator, ~1,100 LOC in the stopped agent's worktree) was parked by maintainer stop and is **replanned into ROADMAP Phase 2.7's playground expansion**, to be relaunched fresh against the post-E/F tree (its dirty files predate the E merge and the docs-app restructure) |
| **E — custom style classes + granular rebuilds + wiring (R6-final, R12, R13)** | custom `@Style` classes replace `LegendStates<T>` (kit predefines `InteractiveColors`); per-field `listen:` opt-out, `_field` accessors, `ValueListenable` selectors; extensions-first wiring with the opt-in `_$XBase` two-arg build; terse Ref renames via `refName:` (`ColorRef`, `SizeRef`, `TextRef`, `ShadowRef`, `StateRef`, `TokenRef`) | **landed** (2026-07-10): Ref catalogs renamed via explicit `refName:` (documented Legend-prefix exception); `@Style()` class form emits `<file>.style.g.dart` (member-wise sparse `merge` + value `==` as a mixin, member-wise `_$XLerp` behind the hand-written one-line static redirect, contract diagnostics naming the fix) with detection via a run-level pure-AST style-class index (+ compiled-in kit builtins) — emission stays one-file-in/one-file-out; `LegendStates<T>`, its `.states` lift, the generator category, and the `withDerived` `of()` post-hook are **deleted** (derivation lives at `pick`/`resolve` call sites — cascade-on-override now works by design, closing the step-C note); `InteractiveColors` + hand-written `pick`/`resolve` extensions replace every per-state field 1:1 (25 step-C pins green unchanged). R12: `LegendTheme`/`LegendThemeOverride` become `InheritedModel`s over **comparator-carrying const aspects** (selector = the field resolved through registry+defaults; no central key table, registry stays open; aspect-less `of` keeps whole-object behavior); generated `of()` reads dependency-free and registers one aspect per `listen: true` field, `_<field>(context)` accessors register just their own, `XThemeListenables.<field>(Listenable, LegendThemeData Function())` bind the distinct-until-changed `LegendThemeSelector` to the generic pair (no kit controller yet — Phase D); lerp identity fast-paths per the audit amendment (tokens + style classes). R13 rev 2 + opt-in base: `_theme` hook kept; auto-detected State `theme` getter extension (skip on not-found/ambiguous); explicit `_$XThemeState` mixin (stateful only — `on State<X>` violates the bound for stateless); always-emitted `_$XBase extends LegendStatelessWidget<XTheme>` (kit custom element, const-able, hot-reload safe) — `LegendDivider` is the living opt-in-base documentation, the five stateful components adopt the getter. Consumer proofs: `BalanceCard` gains the custom `BalanceAccent` style class + member-wise resolution test; probe fixtures cover all five R12 guarantees and all four wiring forms through generated code. legend_gen suite 87 → 115, kit 144 → 168 |
| **F — remaining ports & primitives** | [RFC-003](RFC-003-LEGEND-BODY.md) `LegendBody` + `LegendSliver*` helpers + docs-site migration; `LegendFieldCore` extraction; `LegendInteractive` vocabulary (secondary-tap, focus-tap); vertical menu port | **landed** (2026-07-10): `LegendBody` always-`CustomScrollView` core (`.pinnedFooter`/`.slivers`/`.fixed`) + `LegendSliverPinnedHeader` (own `RenderSliver`) / `LegendSliverSection` / `.asSliver`; docs site migrated onto `LegendBody(maxContentWidth: 860)`; `LegendFieldCore` split out of `LegendTextField` (public API unchanged); `LegendInteractive` gained `onSecondaryTap`/`onLongPress` (local anchor) + `onHoverChange` (Enter/Space already existed), `LegendContextMenu` rewired off its raw `GestureDetector`; `LegendVerticalMenu` on `LegendInteractive`+`LegendSurface`, expandable sections from the shared `LegendNavItem` (added `children`), no `addPostFrameCallback`. Regression tests for every legacy bug closed (controller leak, appBar-in-sliver-mode, half-split padding, keyboard inset, disabled-inert vocabulary, vertical-tile setState storm). **Deferred**: `.refresh`/`LegendSliverRefresh` (RFC-003 §4). Kit suite 176 → 199 |
