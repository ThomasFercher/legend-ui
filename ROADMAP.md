...
- [x] **Wave 2 — structural & disclosure** (landed 2026-07-22, 8/8): `LegendRadioGroup`/`LegendRadio` (roving focus/tab-stop, RTL arrows; 25 tests), `LegendSlider` (drag + tap-to-position, divisions, slider semantics; 14), `LegendDrawer` (start/end/bottom over `LegendModalRoute`, drag-dismiss; 13), `LegendMenu` (sealed entry model on `LegendPopover`, keyboard highlight; 21), `LegendNumberField` (decimal formatter, clamp-on-commit, hold-repeat steppers; 16), `LegendEmpty` (12), `LegendCombobox` (typeahead on `LegendFieldCore` + overlay; 12), `LegendAccordion` (single/multi-open over `LegendExpandable`, extended in place; 8+2) — kit suite 377 → 505, example 17 → 25. Flagged: `LegendDrawer.show` vs top-level `showLegend…` naming; sibling overlay panels lack `IntrinsicWidth` (Menu pins it)
- [x] **Wave 3 — domain & specialized** (landed 2026-07-22, 8/9 — `LegendQrCode` parked pending dep approval): `LegendMarkdown` renderer (pulled forward: AST via approved `markdown: ^7.3.1`, `LegendMarkdownSyntax` custom inline+block registry, selectable spans; 17 tests), `LegendSplitPane` (draggable divider, min-px bounds, RTL keys, breakpoint collapse; 19), `LegendCopyButton`+`LegendAddress` (platform-confirmed copy, live-region feedback, deterministic truncation; 17), `LegendPinField` (one hidden field → N painted cells, paste-fill; 17), `LegendStat` (delta arrows, fromChange; 15), `LegendSteps`/`LegendTimeline` (25), `LegendBreadcrumb`/`LegendPagination` (windowing, RTL; 24), `LegendCodeBlock` (copy + highlighter hook; 9) — kit 522 → 648, example 26 → 35
- [x] **Editor track — `LegendMarkdownEditor`** (landed 2026-07-22, per [RFC-005](docs/RFC-005-MARKDOWN-EDITOR.md)): `LegendMarkdownEditingController` with style-only `buildTextSpan` over unchanged source (exact round-trip proven), incremental per-line tokenizer with fence carry-over and cache reuse, `LegendMarkdownEdits` pure transforms (list continuation, Tab indent/outdent, Cmd/Ctrl+B/I delimiter toggle), highlight hook added to the shared `LegendMarkdownSyntax` registry (one registration highlights in the editor AND renders in `LegendMarkdown`), `LegendFieldCore` extended multiline (additive). 42 kit tests + gallery knob test; kit suite 648 → 690, example 36. Deferred: IME soft-keyboard Enter continuation, scroll-synced preview, fence auto-close
- [x] **Playground expansion** (landed 2026-07-22; absorbed parked Phase 2.6 D, relaunched fresh against the current tree): manifest-driven **Theme explorer** page — a registry over all 54 committed `*.docs.g.dart` manifests (completeness pinned by a filesystem-scan test), type-dispatched entry editors (Color/double/Duration/EdgeInsets/BorderRadius/TextStyle, read-only fallback), sparse `XThemeNullable` overrides compiled by parameter name (dot-path members grouped into `InteractiveColors`) into the same open components map as the hand knobs, searchable picker, live preview + emitted-source block; playground preview column reorganized into sections with every shipped widget live (switch, dialog, toast, form, context menu, shimmer, expandable, info item, divider, copy button, bottom bar added); per-widget theme-surface tables on every docs page rendered from the manifests (`ManifestPropsTable`); RFC-005 shared-registry custom-syntax demo (editor highlight + renderer span from one registration). Example suite 36 → 44. Remaining nice-to-have: a hand-curated token/seed reference section (token classes have no generated manifests yet)
