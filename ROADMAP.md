...
- [ ] **Wave 3 — domain & specialized**: ~~`LegendMarkdown` renderer~~ (pulled forward, landed 2026-07-22 as the editor track's first slice: AST via approved `markdown: ^7.3.1`, `LegendMarkdownSyntax` custom inline+block registry, 16 themed fields, selectable spans; 17 tests — kit 505 → 522, example 25 → 26), `LegendSplitPane`, copy/address-truncation utilities, `LegendQrCode` (dep approval per-case), `LegendPinField`, `LegendStat`, `LegendSteps`/`LegendTimeline`, `LegendBreadcrumb`/`LegendPagination`, `LegendCodeBlock`
