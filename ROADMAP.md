...
- [ ] **Wave 0 — foundations** (in flight): `LegendSelectionControl` (shared checked/pressed/focus base for checkbox/radio/segmented/selectable tiles) + `LegendPopover` (positioned-surface component over the `LegendAnchoredOverlay` engine)
- [ ] **Wave 1 — shared high-frequency core** (queued behind Wave 0): `LegendListItem`/`LegendList` (the #1 gap in all three research lenses), `LegendTabs`, `LegendCheckbox`, `LegendChip`, `LegendBadge`, `LegendAvatar`, `LegendTooltip`, `LegendBanner`, `LegendProgress`, `LegendSegmented`
...
- [ ] **Editor track — `LegendMarkdownEditor`** (decided 2026-07-11, maintainer: **in-house**): built on `LegendFieldCore`/`EditableText`, existing markdown *parser* packages acceptable as deps, must support **custom/extensible syntax** (domain-specific notations) beyond CommonMark; landscape research (engines vs parser packages, web viability, extension APIs) in flight → architecture proposal before build
